...
function tambahData() {
    ...
    const gaji = hitungGaji(lokasi, jumlah);
...
function yen(nilai) {
    ...
    return "¥" + Number(nilai).toLocaleString("ja-JP");
...
function render() {
    ...
    db.forEach((item, index) => {
        ...
        const dataLokasi =
            lokasiDb.find(x => x.nama === item.lokasi);
    ...
    Object.keys(dataPerTanggal)
        ...
        .forEach(tanggal => {
            ...
            if (bulanFilter && !tanggal.startsWith(bulanFilter)) {
                return;
            }
            ...
            dataPerTanggal[tanggal].forEach(item => {

                totalHari += item.gaji;

                const jenis =
                    lokasiDb.find(x => x.nama === item.lokasi)?.jenis || "kamar";
...
`;
            ...
            const terbuka = tanggalTerbuka[tanggal];
...
`;
    ...
    document.getElementById("riwayat").innerHTML = htmlRiwayat;
        // ===============================
    // TOTAL
    // ===============================
    ...
    Object.keys(rekapLokasi).forEach(namaLokasi => {
...
<div class="rekap-row">
    ...
    <span>${yen(rekapLokasi[namaLokasi])}</span>
...
`;
    ...
    document.getElementById("rekapLokasi").innerHTML =
        htmlRekap;
...
<span>${yen(dataPerBulan[bulan].pemasukan)}</span>
...
`;
    ...
    db.forEach(item => {
        ...
        if (item.tanggal !== tanggal) return;
